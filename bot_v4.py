#!/usr/bin/env python3
# Binance Paper Trading Bot v4 — GitHub Actions one-shot — SIMULATION ONLY
...
TRADES_FILE = "paper_trades_v4.csv"
...
def http_get(path, params=None):
    ...
    req = Request(url, headers={"User-Agent":"paper-bot-v4/1.0"})
...
def main():
    print("=" * 72)
    print("BINANCE PAPER TRADING BOT v4 — GITHUB ACTIONS — SIMULATION ONLY")
    print("One scheduled scan per run. No API key. No real orders.")
    print("=" * 72)
    ...
    if free_slots > 0 and eligible:
        ...
        for a in eligible[:free_slots]:
            buy(state, a["symbol"], a["price"], a["score"], a["reason"])
    ...
    print("Run complete. State saved for next scheduled run.")
