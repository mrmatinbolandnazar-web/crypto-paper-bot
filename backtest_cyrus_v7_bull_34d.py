...
CACHE = Path(".backtest_cache_v5_2")
REPORT = Path("backtest_cyrus_v7_bull_34d_report.txt")
TRADES = Path("backtest_cyrus_v7_bull_34d_trades.csv")
...
def fetch_5m(symbol, start_ms, end_ms):
    ...
    fallback = CACHE / f"{symbol}_5m_20260525_20260827.json"
    if fallback.exists():
        try:
            data = json.loads(fallback.read_text())
            sliced = [x for x in data if start_ms <= int(x["close_time"]) <= end_ms]
            if sliced:
                return sliced
        except Exception:
            pass
...
def main():
    ...
    end_ms = floor5(datetime(2026, 8, 27, 14, 5, tzinfo=timezone.utc))
    ...
    lines = [
        "=" * 72,
        "CYRUS TRADER V7 - BULL MARKET 34-DAY TEST",
        "SIMULATION ONLY",
        "=" * 72,
