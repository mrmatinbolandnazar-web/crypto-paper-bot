#!/usr/bin/env python3
# Binance Paper Trading Bot V5.2C BALANCED — SIMULATION ONLY
...
CONFIG = {
    ...
    "trend_mom15_min": 0.0030,
    "trend_mom30_min": 0.0050,
    "trend_volume_ratio_min": 1.20,
    "trend_rsi_max": 70.0,
...
STATE_FILE = "paper_state_v5_2_balanced.json"
TRADES_FILE = "paper_trades_v5_2_balanced.csv"
...
def analyze(symbol, source="CORE", trend_meta=None):
    ...
    tf1h_not_bearish = bool(
        tf1h
        and tf1h["rsi"] >= 42.0
        and tf1h["mom3"] >= -0.0030
        and tf1h["price"] >= tf1h["ema_slow"] * 0.997
    )

    mtf_ok = tf15_ok and tf1h_not_bearish and chase_ok and spread_ok
    ...
    return {
        ...
        "tf15_ok": tf15_ok,
        "tf1h_ok": tf1h_ok,
        "tf1h_entry_ok": tf1h_not_bearish,
    ...
    if btc and btc["mom15"] <= CONFIG["btc_15m_crash_pct"]:
        ...
        if breadth >= CONFIG["strong_breadth"] and btc_ok and eth_ok:
            ...
            if breadth >= CONFIG["probe_breadth"]:
                ...
                raw_regime = "CAUTIOUS"
                reason = "btc_not_confirmed_cautious"
        else:
            raw_regime = "NORMAL"
...
def main():
    print("=" * 96)
    print("BINANCE PAPER TRADING BOT V5.2C BALANCED — PARALLEL SIMULATION ONLY")
    ...
    for symbol, source in source_map.items():
        try:
            ...
            print(
                ...
                f"TF1H={'Y' if a.get('tf1h_entry_ok') else 'N'} "
    ...
    for a in analyses:
        ...
        if not entry_ready:
            ...
            near_misses.append({
                ...
                "tf1h": a.get("tf1h_entry_ok", False),
    ...
    print("Run complete. V5.2C Balanced state saved for next scheduled run.")
