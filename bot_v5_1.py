#!/usr/bin/env python3
# Binance Paper Trading Bot V5.1 — SIMULATION ONLY
...
CONFIG = {
    "interval": "5m",
    "candle_limit": 180,
    "core_target_count": 50,
    "trend_scan_count": 12,
    "trend_prefilter_count": 24,
    ...
    "rsi_min": 47.0,
    "rsi_max": 68.0,
    "momentum_5m_floor": -0.0005,
    "momentum_15m_min": 0.0010,
    "momentum_30m_min": -0.0010,
    "volume_ratio_min": 0.70,

    "trend_24h_change_min": 1.5,
    "trend_24h_change_max": 15.0,
    "trend_quote_volume_min": 8_000_000.0,
    "trend_mom15_min": 0.0035,
    "trend_mom30_min": 0.0050,
    "trend_volume_ratio_min": 1.10,
    "trend_rsi_max": 69.0,
    "trend_score_bonus_required": 0.35,

    "risk_off_breadth": 0.18,
    "weak_breadth": 0.40,
    "strong_breadth": 0.70,
    "btc_15m_crash_pct": -0.0060,

    "min_stop_pct": 0.0045,
    "max_stop_pct": 0.0120,
    "atr_stop_multiple": 1.20,
    "reward_risk_normal": 1.85,
    "reward_risk_trend": 2.00,
    "min_take_profit_pct": 0.0100,
    "max_take_profit_pct": 0.0300,
    ...
    "symbol_cooldown_loss_minutes": 60,
    "loss_streak_hard_pause": 3,
    "global_pause_minutes": 45,
}

STATE_FILE = "paper_state_v5_1.json"
TRADES_FILE = "paper_trades_v5_1.csv"
...
def http_get(path, params=None):
    ...
    req = Request(url, headers={"User-Agent": "paper-bot-v5.1/1.0"})
...
def build_core_universe(info_map, ticker_map):
    chosen, missing = [], []
    for symbol in PRIMARY_CORE_50:
        if symbol in info_map:
            chosen.append(symbol)
        else:
            missing.append(symbol)

    for symbol in RESERVE_PRIORITY:
        if len(chosen) >= CONFIG["core_target_count"]:
            break
        if symbol in info_map and symbol not in chosen:
            chosen.append(symbol)

    if len(chosen) < CONFIG["core_target_count"]:
        liquid = sorted(
            ((s, ticker_map.get(s, {}).get("quote_volume", 0.0))
             for s in info_map if s not in chosen),
    ...
    if len(chosen) < CONFIG["core_target_count"]:
        raise RuntimeError(
            f"Only {len(chosen)} eligible USDT spot pairs available; "
            f"cannot build {CONFIG['core_target_count']} core pairs."
        )

    chosen = chosen[:CONFIG["core_target_count"]]
    replacements = [s for s in chosen if s not in PRIMARY_CORE_50]
    return chosen, missing, replacements
...
def build_trend_watch(core_symbols, ticker_map):
    ...
    for symbol, t in ticker_map.items():
        ...
        if qv < CONFIG["trend_quote_volume_min"]:
            continue
        if not (CONFIG["trend_24h_change_min"] <= chg <= CONFIG["trend_24h_change_max"]):
            continue
        rank = chg + 0.35 * math.log10(max(qv, 1.0))
        ranked.append((rank, qv, chg, symbol))

    ranked.sort(reverse=True)
    top = ranked[:CONFIG["trend_prefilter_count"]]
    symbols = [x[3] for x in top[:CONFIG["trend_scan_count"]]]
    meta = {x[3]: {"quote_volume": x[1], "change_pct": x[2]} for x in top}
    return symbols, meta
...
def analyze(symbol, source="CORE", trend_meta=None):
    ...
    return {
        ...
        "atr_pct": atr, "score": score, "core_entry_ok": core_entry_ok,
...
def choose_exit_params(a, source):
    ...
    rr = CONFIG["reward_risk_trend"] if source == "TREND" else CONFIG["reward_risk_normal"]
    tp_pct = clamp(
        max(CONFIG["min_take_profit_pct"], stop_pct * rr),
        CONFIG["min_take_profit_pct"], CONFIG["max_take_profit_pct"],
    )
    trail_trigger = max(CONFIG["min_trailing_trigger_pct"], stop_pct * 1.10)
    trail_distance = max(CONFIG["min_trailing_distance_pct"], stop_pct * 0.60)
    be_trigger = max(CONFIG["min_breakeven_trigger_pct"], stop_pct * 0.90)
...
def buy(state, a, spend_usdt, reason):
    ...
    qty = (spend - fee) / a["price"]
    state["cash_usdt"] -= spend
    state["total_fees_usdt"] += fee
    state["positions"][symbol] = {
        "source": source, "entry_price": a["price"], "qty": qty,
        "cost_usdt": spend, "entry_fee_usdt": fee, "peak_price": a["price"],
    ...
    log_trade(symbol, "BUY", source, a["price"], qty, spend, fee, 0.0,
              a["score"], stop_pct, tp_pct, reason)
    print(
        f">>> BUY {symbol} [{source}] @ {a['price']:.8f} | ${spend:.2f} paper | "
...
def sell(state, symbol, price, reason):
    ...
    log_trade(
        symbol, "SELL", pos.get("source", "CORE"), price, pos["qty"], gross,
...
def classify_regime(core_analyses):
    if not core_analyses:
        return "RISK_OFF", 0.0, "no_data"
    positive = 0
    for a in core_analyses:
        votes = int(a["ema_fast"] > a["ema_slow"]) + int(a["mom15"] > 0) + int(a["mom30"] > 0)
        if votes >= 2:
            positive += 1
    breadth = positive / len(core_analyses)
    btc = next((a for a in core_analyses if a["symbol"] == "BTCUSDT"), None)

    if btc and btc["mom15"] <= CONFIG["btc_15m_crash_pct"]:
        return "RISK_OFF", breadth, "btc_fast_drop"
    if breadth < CONFIG["risk_off_breadth"]:
        return "RISK_OFF", breadth, "breadth_risk_off"
    if breadth < CONFIG["weak_breadth"]:
        return "WEAK", breadth, "breadth_weak"
    if btc and btc["ema_fast"] < btc["ema_slow"]:
        if breadth >= CONFIG["strong_breadth"]:
            return "NORMAL", breadth, "strong_breadth_but_btc_soft"
        return "WEAK", breadth, "btc_trend_soft"
    if breadth >= CONFIG["strong_breadth"]:
        return "STRONG", breadth, "broad_strength"
    return "NORMAL", breadth, "market_normal"


def risk_profile(regime, loss_streak):
    profiles = {
        "RISK_OFF": {"max_positions": 0, "min_score": 99.0, "size_fraction": 0.0},
        "WEAK": {"max_positions": 1, "min_score": 6.65, "size_fraction": 0.10},
        "NORMAL": {"max_positions": 2, "min_score": 6.15, "size_fraction": 0.15},
        "STRONG": {"max_positions": 4, "min_score": 5.85, "size_fraction": 0.20},
    }
    p = dict(profiles[regime])
    if loss_streak == 1 and regime != "RISK_OFF":
        p["min_score"] += 0.15; p["size_fraction"] *= 0.85
    elif loss_streak == 2 and regime != "RISK_OFF":
        p["min_score"] += 0.40; p["size_fraction"] *= 0.60
        p["max_positions"] = min(p["max_positions"], 1)
    p["max_positions"] = min(p["max_positions"], CONFIG["absolute_max_positions"])
    return p
...
def main():
    print("=" * 96)
    print("BINANCE PAPER TRADING BOT V5.1 — GITHUB ACTIONS — SIMULATION ONLY")
    ...
    regime, breadth, regime_reason = classify_regime(core_analyses)
    ...
    print("Run complete. V5.1 state saved for next scheduled run.")
