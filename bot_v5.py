...
# Conservative upgrade of v4: scans a curated 50-coin USDT spot universe,
...
# Curated 50-coin universe selected for established liquidity and market history.
CURATED_50 = [
    ...
    "HBARUSDT","XLMUSDT","ICPUSDT","AAVEUSDT","INJUSDT",
    "RUNEUSDT","SEIUSDT","TIAUSDT","FETUSDT","RENDERUSDT",
    "WIFUSDT","BONKUSDT","JUPUSDT","ALGOUSDT","VETUSDT",
    "POLUSDT","CAKEUSDT","ONDOUSDT","TAOUSDT","ENAUSDT",
    "PENDLEUSDT","WLDUSDT","STXUSDT","GRTUSDT","IMXUSDT",
]
...
def available_symbols():
    """Return the curated V5 list, skipping pairs that Binance reports unavailable."""
    try:
        info = http_get("/api/v3/exchangeInfo")
        allowed = {
            x.get("symbol") for x in info.get("symbols", [])
            if eligible_spot_symbol(x)
        }
        chosen = [s for s in CURATED_50 if s in allowed]
        missing = [s for s in CURATED_50 if s not in allowed]
        if missing:
            print("Universe note: unavailable curated pairs skipped: " + ", ".join(missing))
        if chosen:
            return chosen
    except Exception as e:
        print(f"Universe validation warning: {e}; using curated list directly.")
    return CURATED_50.copy()
...
def main():
    ...
    print("Curated 50 USDT spot pairs | closed candles | strict entries | risk gate | no real orders")
    print("=" * 88)
    ...
    # Keep managing an open position even if its symbol later becomes unavailable
    # or the curated universe changes.
    scan_symbols = list(dict.fromkeys(symbols + list(state["positions"].keys())))
    ...
    market_analyses = [a for a in analyses if a["symbol"] in universe_set]
    market_ok, breadth, market_reason = market_regime(market_analyses)
