...
# Conservative upgrade of v4: scans a curated universe of 50 established USDT spot pairs,
...
# Fixed curated universe: established, liquid names only.
CURATED_SYMBOLS = [
...
def available_symbols():
    """Return curated symbols that are currently active for Binance spot trading."""
    try:
        ...
        active = {
            ...
            and x.get("isSpotTradingAllowed") is not False
        }
        chosen = [s for s in CURATED_SYMBOLS if s in active]
        missing = [s for s in CURATED_SYMBOLS if s not in active]
        if missing:
            print("Unavailable curated pairs skipped: " + ", ".join(missing))
        return chosen
    except Exception as e:
        print(f"Universe check warning: {e}")
        # If exchangeInfo is temporarily unavailable, let per-symbol requests decide.
        return CURATED_SYMBOLS.copy()
...
def main():
    ...
    print("50 curated USDT spot pairs | closed candles | strict entries | risk gate | no real orders")
    print("=" * 88)
    ...
    # Always keep managing any legacy/open position even if its pair is later removed from the curated universe.
    for open_symbol in state.get("positions", {}):
        if open_symbol not in symbols:
            symbols.append(open_symbol)
